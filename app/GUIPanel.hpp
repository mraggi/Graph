...
struct GuiElement
{
    GuiElement(std::string name_, const sf::Color& color_)
        : name(std::move(name_)), color(color_)
    {}
    std::string name;
    sf::Color color;
...
public:
    ...
    std::string GetString() const override
    {
        ...
        out << name << " [" << decreaseKey << "/" << increaseKey
            << "]: " << *var;
        return out.str();
    }
};
...
struct Checkbox : GuiElement
{
    ...
    void Activate() override { *var = !(*var); }
    ...
    ShapeType Shape() const override { return ShapeType::CircleShape; };
};
...
public:
    ...
    void Activate() override { f(); }

    ShapeType Shape() const override { return ShapeType::BoxShape; };
...
public:
    ...
    // void Watch(real var, const std::string& name, sf::Color col =
    // sf::Color(100, 100, 255));

    // void Watch(const std::string& var, sf::Color col = sf::Color(100, 255,
    // 10));
...
private:
    ...
    struct key_hash
    {
        size_t operator()(const sf::Keyboard::Key& key) const
        {
            return static_cast<size_t>(key);
        }
    };

    std::unordered_map<sf::Keyboard::Key, std::vector<GuiElement*>, key_hash>
      shortcuts{};
...
void GUIPanel::AddController(T& var,
                             ...
                             const sf::Color& color)
{
    gui_elements.emplace_back(
      new VarController<T>(var, incrKey, decrKey, stepsize, name, color));
    shortcuts[incrKey].emplace_back(gui_elements.back().get());
    shortcuts[decrKey].emplace_back(gui_elements.back().get());
}

template <class T>
void GUIPanel::AddWatcher(T& var,
                          const std::string& name,
                          const sf::Color& color)
{
