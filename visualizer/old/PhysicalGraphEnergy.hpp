...
public:
    ...
    PhysicalGraph(const Graph& g)
        : A(adjacency_matrix(g)), X(g.num_vertices()*2), E(g.edges())
    {
        PA.AddElement(repelling_force,
                      "Repelling Force",
                      0.1,
                      sf::Keyboard::T,
                      sf::Keyboard::Y);
        std::generate(
          X.begin(), X.end(), []() { return random_real(0.0, 500.0); });
    }
    ...
    PhysicalGraph()
    {
        PA.AddElement(repelling_force,
                      ...
                      sf::Keyboard::T,
                      sf::Keyboard::Y);
        PA.AddElement(repelling_threshold,
                      "Repelling Thresh",
                      1,
                      sf::Keyboard::G,
                      sf::Keyboard::H);
