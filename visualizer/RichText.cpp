...
RichText::RichText(const RichText& other)
{
	...
	for (std::map<String, Color>::const_iterator it = other.colors.begin();
		 it != other.colors.end();
		 ++it)
	{
...
String RichText::getString() const { return string; }

String RichText::getSource() const { return source; }
...
unsigned int RichText::getCharacterSize() const { return characterSize; }
...
const Font* RichText::getFont() const { return font; }
...
FloatRect RichText::getLocalBounds() const { return bounds; }
...
void RichText::addColor(const String& name, const Color& color) { colors[name] = color; }

void RichText::addColor(const String& name, unsigned argbHex) { colors[name] = getColor(argbHex); }
...
Color RichText::getColor(const String& source) const
{
...
Color RichText::getColor(unsigned argbHex) const
{
	argbHex |= 0xff000000;
	return Color(
	  argbHex >> 16 & 0xFF, argbHex >> 8 & 0xFF, argbHex >> 0 & 0xFF, argbHex >> 24 & 0xFF);
}
}
