...
bool Intersect(const Convex& A, const Convex& B, Polygon* intersection)
{
	...
	switch (Atype)
	{
		case (shape_Line):
			...
			{
				case (shape_Line):
					toReturn = Intersect(static_cast<const Line&>(A), static_cast<const Line&>(B), &pointinter);
					if (intersection != NULL)
						intersection->CreateFromPoint(pointinter);
					return toReturn;
					return toReturn;
					break;
				case (shape_Segment):
					toReturn = Intersect(static_cast<const Line&>(A), static_cast<const Segment&>(B), &pointinter);
					if (intersection != NULL)
						intersection->CreateFromPoint(pointinter);
					return toReturn;
					return toReturn;
					break;
				case (shape_Ray):
					toReturn = Intersect(static_cast<const Line&>(A), static_cast<const Ray&>(B), &pointinter);
					if(intersection != NULL) intersection->CreateFromPoint(pointinter);
					return toReturn;
					break;
				case (shape_Circle):
					toReturn = Intersect(static_cast<const Line&>(A), static_cast<const Circle&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Box):
					toReturn = Intersect(static_cast<const Line&>(A), static_cast<const Box&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Polygon):
					toReturn = Intersect(static_cast<const Line&>(A), static_cast<const Polygon&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
			};
		...
		case (shape_Segment):
			...
			{
				case (shape_Line):
					toReturn = Intersect(static_cast<const Segment&>(A), static_cast<const Line&>(B), &pointinter);
					if(intersection != NULL) intersection->CreateFromPoint(pointinter);
					return toReturn;
					break;
				case (shape_Segment):
					toReturn = Intersect(static_cast<const Segment&>(A), static_cast<const Segment&>(B), &pointinter);
					if(intersection != NULL) intersection->CreateFromPoint(pointinter);
					return toReturn;
					break;
				case (shape_Ray):
					toReturn = Intersect(static_cast<const Segment&>(A), static_cast<const Ray&>(B), &pointinter);
					if(intersection != NULL) intersection->CreateFromPoint(pointinter);
					return toReturn;
					break;
				case (shape_Circle):
					toReturn = Intersect(static_cast<const Segment&>(A), static_cast<const Circle&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Box):
					toReturn = Intersect(static_cast<const Segment&>(A), static_cast<const Box&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Polygon):
					toReturn = Intersect(static_cast<const Segment&>(A), static_cast<const Polygon&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
			};
		...
		case (shape_Ray):
			...
			{
				case (shape_Line):
					toReturn = Intersect(static_cast<const Ray&>(A), static_cast<const Line&>(B), &pointinter);
					if(intersection != NULL) intersection->CreateFromPoint(pointinter);
					return toReturn;
					return toReturn;
					return toReturn;
					break;
				case (shape_Segment):
					toReturn = Intersect(static_cast<const Ray&>(A), static_cast<const Segment&>(B), &pointinter);
					if(intersection != NULL) intersection->CreateFromPoint(pointinter);
					return toReturn;
					break;
				case (shape_Ray):
					toReturn = Intersect(static_cast<const Ray&>(A), static_cast<const Ray&>(B), &pointinter);
					if(intersection != NULL) intersection->CreateFromPoint(pointinter);
					return toReturn;
					break;
				case (shape_Circle):
					toReturn = Intersect(static_cast<const Ray&>(A), static_cast<const Circle&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Box):
					toReturn = Intersect(static_cast<const Ray&>(A), static_cast<const Box&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Polygon):
					toReturn = Intersect(static_cast<const Ray&>(A), static_cast<const Polygon&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
			};
		...
		case (shape_Circle):
			switch (Btype)
			{
				case (shape_Line):
					...
					toReturn = Intersect(static_cast<const Circle&>(A), static_cast<const Line&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Segment):
					toReturn = Intersect(static_cast<const Circle&>(A), static_cast<const Segment&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Ray):
					toReturn = Intersect(static_cast<const Circle&>(A), static_cast<const Ray&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Circle):
					toReturn = Intersect(static_cast<const Circle&>(A), static_cast<const Circle&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Box):
					...
					break;
				case (shape_Polygon):
					toReturn = Intersect(static_cast<const Circle&>(A), static_cast<const Polygon&>(B));
					return toReturn;
					break;
			};
			break;
		case (shape_Box):
			...
			{
				case (shape_Line):
					toReturn = Intersect(static_cast<const Box&>(A), static_cast<const Line&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Segment):
					toReturn = Intersect(static_cast<const Box&>(A), static_cast<const Segment&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Ray):
					toReturn = Intersect(static_cast<const Box&>(A), static_cast<const Ray&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Circle):
					...
					break;
				case (shape_Box):
					toReturn = Intersect(static_cast<const Box&>(A), static_cast<const Box&>(B), &boxinter);
					if(intersection != NULL) intersection->CreateFromBox(boxinter);
					return toReturn;
					break;
				case (shape_Polygon):
					toReturn = Intersect(static_cast<const Box&>(A), static_cast<const Polygon&>(B));
					return toReturn;
					break;
			};
			break;
		case (shape_Polygon):
			...
			{
				case (shape_Line):
					toReturn = Intersect(static_cast<const Polygon&>(A), static_cast<const Line&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Segment):
					toReturn = Intersect(static_cast<const Polygon&>(A), static_cast<const Segment&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Ray):
					toReturn = Intersect(static_cast<const Polygon&>(A), static_cast<const Ray&>(B), &segmentinter);
					if(intersection != NULL) intersection->CreateFromSegment(segmentinter);
					return toReturn;
					break;
				case (shape_Circle):
					toReturn = Intersect(static_cast<const Polygon&>(A), static_cast<const Circle&>(B));
					return toReturn;
					break;
				case (shape_Box):
					toReturn = Intersect(static_cast<const Polygon&>(A), static_cast<const Box&>(B));
					return toReturn;
					break;
				case (shape_Polygon):
					toReturn = Intersect(static_cast<const Polygon&>(A), static_cast<const Polygon&>(B));
					return toReturn;
					break;
			};
			break;
...
bool Intersect(const Line& A, const Polygon& B, Segment* intersection)
{
	...
	for (nuint i = 0; i < B.NumPoints()+1; ++i)
	{
		if (B[i].IsToTheLeftOfLine(p,q) != left)
		{
			...
			{
				cout << "ERROR IN Intersect(const Line& A, const Polygon& B, Segment* intersection)" << endl;
			}
...
bool Intersect(const Ray& A, const Circle& B, Segment* intersection)
{
	...
	if (!A.SegmentInLineIntersectsMe(intersection->Origin(), intersection->End(), intersection)) return false;
