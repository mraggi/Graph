...
Polygon::Polygon(const Segment& segment)
    : FConvex(segment.Position()), m_vPoints(2)
{
...
real Polygon::Perimeter() const
{
    ...
    for (size_t i = 0; i + 1 < m_vPoints.size(); ++i)
    {
...
bool Polygon::Intersects(const Line& other,
                         Point& intersection,
                         Point& /*normal*/) const
{
    // TODO(mraggi): Normal
...
bool Polygon::Intersects(const Line& other, Segment& intersection) const
{
    ...
    for (int i = 0; i < B.NumPoints() + 1; ++i)
    {
        if (B[i].IsToTheLeftOfLine(p, q) != left)
        {
            ...
            {
                std::cerr << "ERROR IN Intersect(const Line& A, const Polygon& "
