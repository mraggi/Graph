...
Segment::Segment(const Point& pointA, const Point& pointB) :																		  
			FConvex((pointA+pointB)/2),
			m_vDirection((pointB-pointA)/2)
{
	
}

Segment::Segment() : FConvex(Point(0,0)),
							m_vDirection(Point(1,0))
{
...
{
	if (angle == m_vDirection.Angle()) return End();
	if (angle == -m_vDirection.Angle()) return Origin();
