...
bool Circle::PerpendicularSegmentOfPoint(const Point& point,
                                         Segment* perp) const
{
...
{
    Point p =
      Position().ProjectionToLine(other.Position(), other.SecondPoint());
    return Intersects(p);
}
...
bool Circle::Intersects(const Box& other) const
{
...
bool Circle::Intersects(const Polygon& other) const
{
    return other.Intersects(*this);
}
...
bool Circle::Intersects(const Line& other, Segment& intersection) const
{
    ...
    real iy1 = (-D*dx + std::abs(dy)*sqDelta)/dr2;

    real ix2 = (D*dy - sgn*dx*sqDelta)/dr2;
    real iy2 = (-D*dx - std::abs(dy)*sqDelta)/dr2;
    ...
    intersection.SetOriginAndEnd(Point(ix1, iy1) + Position(),
                                 Point(ix2, iy2) + Position());

    return true;
}

bool Circle::Intersects(const Line& other,
                        Point& intersection,
                        Point& normal) const
{
