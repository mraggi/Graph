...
void DrawGraph(Client& C, const Graph& G, const vector<Point>& P, const vector<sf::Color>& vertex_colors)
{
	for (vertex_t n = 0; n < G.num_vertices(); ++n)
	{
		for (auto u : G.neighbors(n))
...
void DrawGraph(Client& C, const DiGraph& G, const vector<Point>& P, const vector<sf::Color>& vertex_colors)
{
	for (vertex_t n = 0; n < G.num_vertices(); ++n)
	{
		for (auto u : G.outneighbors(n))
